...
public class JuicetaniumArmor extends ArmorItem {
    public JuicetaniumArmor(IArmorMaterial pMaterial, EquipmentSlotType pSlot) {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable World pWorld, List<ITextComponent> pTooltipComponents, ITooltipFlag pIsAdvanced) {
        ...
        pTooltipComponents.add(new TranslationTextComponent("item." + XtraDrinks.MOD_ID + ".tooltip.juicetanium_armor").withStyle(TextFormatting.YELLOW));
    }
    ...
    public void onArmorTick(ItemStack stack, World world, PlayerEntity player) {
        ...
        if (ArmourPieces == 4) {
            player.addEffect(new EffectInstance(Effects.DAMAGE_BOOST, 40,2));
            player.addEffect(new EffectInstance(Effects.MOVEMENT_SPEED, 40,2));
            player.addEffect(new EffectInstance(Effects.SATURATION, 40,2));
        }
    }
}
