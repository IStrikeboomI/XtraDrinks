...
public class LiquidDehydratorContainer extends Container {
    public final LiquidDehydratorTileEntity blockEntity;
    private ItemStackHandler blockInventory;
    private final IWorldPosCallable ACCESS;

    public LiquidDehydratorContainer(int windowId, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player, IWorldPosCallable access) {
        super(XtraDrinksContainers.LIQUID_DEHYDRATOR_CONTAINER.get(), windowId);
        this.ACCESS = access;
        blockEntity = (LiquidDehydratorTileEntity)player.getCommandSenderWorld().getBlockEntity(pos);
        if (blockEntity != null) {
    ...
    public boolean stillValid(PlayerEntity pPlayer) {
        return stillValid(ACCESS, pPlayer, XtraDrinksBlocks.LIQUID_DEHYDRATOR.get());
    }
}
